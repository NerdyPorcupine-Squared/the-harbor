...
These findings are active recovery evidence, not release validation. Navigation, Home hierarchy, Media Bar, and details remain active recovery work. The V3 owner matrix must pass on the same immutable candidate in both Chrome/Edge Jellyfin Web and Jellyfin Media Player before compatibility can be called fully validated.
