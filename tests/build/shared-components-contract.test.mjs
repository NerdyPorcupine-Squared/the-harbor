...
test("keeps the sanitized fixture local and representative of Jellyfin components", async () => {
  const fixture = await readRepositoryFile(
    "tests/fixtures/jellyfin/shared-components.html",
  );
  ...
  for (const className of requiredClasses) {
    assert.match(fixture, new RegExp(`class="[^"]*\\b${className}\\b`, "u"));
  }
...
test("preserves mobile touch targets and visible interaction states", async () => {
  const componentPaths = [
    ...
    "src/css/components/menus.css",
    "src/css/components/dialogs.css",
  ];
  const componentCss = (
    await Promise.all(componentPaths.map((path) => readRepositoryFile(path)))
  ).join("\n");
  ...
  for (const selector of [
    ...
    for (const state of [":hover", ":active", ":disabled", ":focus-visible"]) {
      assert.match(componentCss, new RegExp(`\\${selector}${state}`, "u"));
    }
  }
});
...
test("reserves brass card framing for focus and selection", async () => {
  ...
  assert.match(cards, /\.card:focus-within[^}]*\.card\.selected[^{}]*\.cardBox\s*\{[^}]*border-color:/su);
  assert.doesNotMatch(
    cards,
    /\.card:hover[^{}]*\.cardBox\s*\{[^}]*border-color:\s*rgb\(184 148 75/su,
  );
});
